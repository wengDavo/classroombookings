...
class Install extends MY_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->library('install/installer');
		...
		if (config_item('is_installed') &&
		    (
			! isset($_SESSION['install_complete'])
	...
	public function config()
	{
		...
		$drivers = $this->installer->get_db_drivers();
		if (empty($drivers)) {
		...
		if ($this->input->post()) {
			...
			$this->form_validation->set_rules('port', 'Port', 'required|is_natural_no_zero');
	...
	private function start_install()
	{
		...
		//
		if (is_file(ROOTPATH . 'local/config.php')) {

			// Config file already present
			$local_config = require(ROOTPATH . 'local/config.php');
			if ( ! is_array($local_config)) {
				show_error("The 'local/config.php' file is not in a recognised format.");
			}
			$db_config = element('database', $local_config, array());
			$db_config['db_debug'] = ENVIRONMENT === 'development';
		...
		$this->installer->set_db_config($db_config);

		$this->installer->set_name($_SESSION['data']['name'] ?? '');

		$this->installer->set_initial_user([
			'username' => $_SESSION['data']['admin_username'],
			'password' => $_SESSION['data']['admin_password'],
		]);

		$res = $this->installer->execute();

		if ( ! $res['success']) {

			$this->errors = $this->installer->get_errors();
			$errors_data = json_encode($this->errors);

			log_message("error", "Install failed: {$errors_data}");

			return false;
		}

		if ( ! $this->install_config_file()) {
			log_message("error", "Install: config failed.");
			return false;
		}

		write_file(ROOTPATH . 'local/installed', date("Y-m-d H:i:s"));
	 ...
	 *
	 */
	private function install_config_file()
	{
		if (is_file(ROOTPATH . 'local/config.php')) {
		...
		$config_tpl = file_get_contents(APPPATH . 'modules/install/resources/config.tpl.php');
		...
		$res = write_file(ROOTPATH . 'local/config.php', $config_contents);
