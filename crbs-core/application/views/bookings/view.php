...
$this->table->set_template([
    'table_open' => '<table class="zebra-table" width="100%" cellpadding="8" cellspacing="0" border="0">',
]);
...
$links_html = empty($links) ? '' : iconbar($links);
...
$info[] = [
    'name' => 'session',
    'label' => 'Session',
    'value' => isset($booking->session->name) ? html_escape($booking->session->name) : '<em>Not assigned</em>',
];
...
$department = ($booking->department && $booking->department->name)
    ? $booking->department
    : ($booking->user && $booking->user->department ? $booking->user->department : null);
if ($department) {
    $info[] = [
        'name' => 'department',
        'label' => 'Department',
...
$info[] = [
    ...
    'value' => isset($booking->department_group->name) ? html_escape($booking->department_group->name) : '<em>Not assigned</em>',
];
if (isset($booking->department_group->size)) {
    $info[] = [
        'name' => 'department_group_capacity',
        'label' => '',
        'value' => html_escape($booking->department_group->size),
    ];
}
...
$info[] = [
    'name' => 'course',
    'label' => 'Course',
    'value' => isset($booking->course->name) ? html_escape($booking->course->name) : '<em>Not assigned</em>',
];
if (isset($booking->course->course_code)) {
    $info[] = [
        'name' => 'course_code',
        'label' => '',
        'value' => html_escape($booking->course->course_code),
    ];
}
if (isset($booking->course->unit)) {
    $info[] = [
        'name' => 'course_unit',
        'label' => 'Unit',
        'value' => html_escape($booking->course->unit),
    ];
}
